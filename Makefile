...
help:
	...
	@echo "  check-all     - Run ALL checks: fmt + lint + test-ci (most thorough)"
	@echo ""
	...
	@echo "  test-ci       - MOST COMPLETE: race + coverage + all packages (CI-identical)"
	@echo "  test-quick    - Fast tests without race detector (-short flag)"
	@echo "  test-unit     - Unit tests only (-tags=unit)"
	@echo "  test-race     - Tests with race detector (same as 'test')"
	@echo "  test-pkg PKG= - Test specific package (e.g., make test-pkg PKG=api)"
	@echo "  test-verbose  - Tests with verbose output"
	@echo "  test-bench    - Run benchmarks"
	@echo "  coverage      - Generate HTML coverage report (opens coverage.html)"
	@echo ""
	...
	@echo "  lint-ci       - Run linters exactly as CI does (5m timeout)"
	@echo "  lint-fast     - Quick lint check for pre-commit (1m timeout)"
	@echo ""
	@echo "🎯 Pre-commit Commands:"
	@echo "  pre-commit      - Quick checks: fmt-check + lint-fast + test-quick"
	@echo "  pre-commit-full - Full checks: fmt-check + lint + test-ci (same as check-all)"
	@echo "  install-hooks   - Install git pre-commit hook"
	@echo "  uninstall-hooks - Remove git pre-commit hook"
	@echo ""
	@echo "⚙️  Development Commands:"
	@echo "  all           - Full pipeline: clean + deps + test + build"
	...
	@echo "  • make test-ci    = CI test job (race + coverage + all packages)"
	@echo "  • make lint-ci    = CI lint job (golangci-lint with 5m timeout)"
	@echo "  • make build-all  = CI build job (linux/darwin/windows, amd64/arm64)"
	@echo ""
	@echo "💡 Which command should I use?"
	@echo "  • Just tests:          make test-ci     (most complete test run)"
	@echo "  • Tests + quality:     make check-all   (fmt + lint + test-ci)"
	@echo "  • Full build pipeline: make all         (clean + deps + test + build)"
	@echo "  • Before committing:   make pre-commit  (quick) or pre-commit-full (thorough)"
	@echo ""
